...
Cgi::Cgi () { this->_body = "body"; }
Cgi::Cgi (Cgi const& cgi) { (void)cgi; }
Cgi::~Cgi () {}

Cgi&			Cgi::operator= (Cgi const& cgi) { (void)cgi; return (*this); }

void			Cgi::setEnv (const std::string& envKey, const std::string& envValue)
{ _env[envKey] = envValue; }
void			Cgi::setCgiExist (const int& exist) { _exists = exist; }
int				Cgi::getCgiExist () { return (_exists); }
std::map<std::string, std::string>	Cgi::getEnv() { return (_env); }
